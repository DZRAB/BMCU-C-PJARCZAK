...
static const _tpu_param TPU_PARAMS[] =
{
    ...
    // 仍保持"软料推力 < 硬料(PLA/PETG)"的特性，只是不再软到吸不动。下列数值为初校值，机台实测可再调。
    // 硬度升序：85A(最软) < 90A < 95A < 68D(最硬,最接近PLA)。
    { _tpu_model::TPU_FOR_AMS,  "GFU98", "TPU for AMS",  0x00u,0xFFu,0xFFu, 50.0f, 58.0f, 2000, 3000, 6000, 650.0f, 600.0f, 600.0f, 650.0f, 0.01f, 600, 450, 55.0f }, // 68D 青（最硬）
    { _tpu_model::TPU_95A_HF,   "GFU00", "TPU 95A HF",   0x00u,0xFFu,0x00u, 45.0f, 54.0f, 2500, 4000, 7000, 580.0f, 540.0f, 540.0f, 580.0f, 0.02f, 650, 470, 50.0f }, // 95A HF 绿
    { _tpu_model::TPU_GEN_AMS,  "GFU02", "Generic TPU",  0xFFu,0x00u,0xFFu, 50.0f, 58.0f, 2000, 3000, 6000, 650.0f, 600.0f, 600.0f, 650.0f, 0.01f, 600, 450, 55.0f }, // 约68D 紫（最硬）
    { _tpu_model::TPU_95A,      "GFU95", "TPU 95A",      0xFFu,0xD0u,0x00u, 45.0f, 54.0f, 2500, 4000, 7000, 560.0f, 520.0f, 520.0f, 560.0f, 0.02f, 700, 480, 50.0f }, // 95A 黄
    { _tpu_model::TPU_90A,      "GFU90", "TPU 90A",      0xFFu,0x80u,0x00u, 40.0f, 50.0f, 3000, 5000, 8000, 500.0f, 460.0f, 460.0f, 500.0f, 0.03f, 800, 520, 45.0f }, // 90A 橙
    { _tpu_model::TPU_85A,      "GFU85", "TPU 85A",      0xFFu,0x00u,0x00u, 35.0f, 46.0f, 3500, 6000, 9000, 440.0f, 400.0f, 400.0f, 440.0f, 0.04f, 900, 560, 40.0f }, // 85A 红（最软）
};
