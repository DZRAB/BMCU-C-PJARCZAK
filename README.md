...
### v3.2-fix105 更新说明
- **修复「手动暂停时 BMCU 不停」**：打印机发来暂停 / 停止指令时，BMCU 若正在自己送料（打印机下发的送料命令，或自动回抽后的送料动作），会无视指令继续转动；现已改为收到停止指令**立刻退出当前送料动作并停机**。实现：`bambu_bus_ams.cpp` 的 `is_stop_on_use` 放宽了仅在 `loaded==ch` 才接受停止的限制（送料时 `loaded=0xFF` 也能停），并新增 `Motion_control_request_stop_dm_autoload()` 在 DM 自动装载送料中即时中止。
- **优化「进料时电机控制」**：原版在料到挤出机入口 / 过五通顶满缓冲头时，要么一直大力死推（电机空转啃料、易误报堵），要么判断不准。因 BMCU 与打印机无「料到哪儿」通讯，改为**时间配合法**——顶满后：① 先用中等力推约 2 秒帮过五通 / 送进挤出机；② 再减到很小力轻顶约 3 秒等打印机拉走（此间缓冲头回落即恢复正常送料、绿灯）；③ 超 5 秒仍满才算真堵、停机报错（红灯）。期间亮**黄灯**（保护避让，非故障），仅真堵亮红灯。实现：`Motion_control.cpp` 的 `pressure_ctrl_on_use` 顶满分支改为三段式，新增 `g_on_use_full_ms[]` 计时。
