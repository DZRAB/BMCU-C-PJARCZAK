...
- **AHT20 版本（v2.0-aht20）额外注意**：
  ...
  - **无 AHT20 时的软件模拟测试（调试用）**：源码内置 `sim_aht20_run()`（`src/sim_aht20.cpp`），在 `main.cpp` 主循环中**每 5 秒**模拟刷新一次温湿度（温度 20~30℃、湿度 30~70% 三角波往返），用于在无硬件时验证上位机能否读到变化的温湿度。该调用在 `main.cpp` 中已加注释标注；**AHT20 实物焊接到位后，务必注释掉 `main.cpp` 中的 `sim_aht20_run();` 这一行**，否则模拟值会每 5 秒覆盖真实传感器读数。此功能仅用于测试，正式发布固件建议去掉。
