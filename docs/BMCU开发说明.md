...
SSD1306 若每次刷新都整行 `clear_line()`（清整行再写）或整屏 `clear()` 会导致肉眼可见黑闪，且全屏重绘每秒多次发 I2C 会抢 AHT20 总线、卡主循环。改造如下（采用**后台显存 framebuffer + 差值刷新**方案）：
- **新增后台显存**：`s_fb[8][128]`（模拟整块 OLED 的 1024 字节 GDDRAM）作为绘制目标；`s_fb_prev[8][128]` 保存「已发送到屏」的上一帧。所有 `show_char`/`show_text`/`clear_line`/`clear` 都**只写 `s_fb`，不再直接发 I2C**。
- **差值刷新 `flush()`**：绘制完成后（tick 末尾）调用一次 `flush()`，逐字节比对 `s_fb` 与 `s_fb_prev`，**仅把变化字节经 I2C 发出**（按页扫描、连续变化段一次 bulk 写）。不变区域零 I2C 流量 → 极大降耗；永不整屏 `clear` → 彻底无黑闪。
- 删除 `tick()` 中**重复的 `switch(s_page)` 块**（会把每页每帧画两遍，是刷新慢+闪屏的重要元凶）。
- RAM 代价：framebuffer 占 2KB（RAM 用量上升约 10%，仍在 20KB 总容量内安全）。
- 注：固定只显抓包页的调试开关 `if (true) { draw_sniffer(); return; }` 下，切页逻辑不执行；恢复正常轮询后切页时有 `clear()`（仅清 framebuffer，不闪），下一次 flush 一次性全量发屏。

**抓包页显示策略（v4.0-tpu 调试：已知正常显示、未知特别记录）**
- **已知指令**：动作相关指令（`on_use`/`stop`/`before_on_use`/`before_pullb`，由 MOT/STU 包 `statu_flag+motion_flag` 判定）进入动作缓存，显示在第 0 行 `R`。周期轮询（SN/VER/RD/RFID/ONL/MC）只计数、不显示，避免刷屏。
- **未知指令（UNK）**：`get_packge_type()` 的 `default` 分支（代码未登记的包，如打印机可能下发的「打印成功 / 暂停 / 停止」等）记入**专用未知缓存** `g_last_unk_*`，第 2 行以 `?` 前缀特别高亮显示其原始片段，第 3 行累计 `?N` 未知计数。这是用户最想抓的内容——用来发现「成功 / 暂停 / 停止」是否真的给 BMCU 发了包。
...
**关于「R 后面总是 SN」**：已解决。SN 属打印机周期轮询，现在被过滤不进显示缓存；`R` 行只显示动作指令，空闲显 `--`。想确认 RX 链路活着看 `r` 计数是否增长即可。

**霸屏功能已移除**：不再有进料/退料顶屏霸屏（`notify_action` 已无视觉作用），收到的信息直接显示在抓包页，无黑闪、无卡顿。

**排查用法**（盯此页）：
- `R` 行长期 `--` 且 `r` 不增长 → 打印机没跟 BMCU 通讯（接线/地址/波特率）。
- `R` 显示 `on_use`/`stop` 跳变 → 打印机在动作前后正确下发指令，BMCU 收到。
- 出现 `? <片段>` 且 `?N` 增长 → 抓到**未知指令**，正是想找的「成功/暂停/停止」等未登记包；记录片段即可反推协议。
...
> 注：v4.0-tpu 调试期已**移除动作霸屏/指令霸屏**（`notify_action` 不再顶屏），改为抓包页持续显示 + 差值刷新。动作信息通过抓包页第 0 行 `R on_use/stop` 自然呈现，不再单独插队页面。
...
- **实测空间（standard/1/0/D/1/GFU02..GFU85 基准，2026-08）**：
  ...
  - **`BMCU_AUTO_REBOOT_ENABLE=1`（开启打印完成软复位）**：在 OLED=1/AHT20=1 全功能基础上再增约 524B（REBOOT=0 基准 Flash 61316 99.8% → REBOOT=1 Flash 61340 99.9%，余量仅 100B）。正因为 Flash 已逼近上限，8x8 字模优化（见 15.7.6）释放的空间才让 REBOOT=1 得以装下。

### 15.7 抓包页与未知指令改造日志（v4.0-tpu 调试期）

本节记录 v4.0-tpu 调试阶段对 OLED 抓包页做的两轮关键改造（对应 commit `6e97f8e`、`bdb5749`）。

#### 15.7.1 第一轮：framebuffer 差值刷新 + 未知指令 UNK 抓取（commit `6e97f8e`）

**问题**：旧驱动 `show_text`/`clear_line` 直接往 SSD1306 发 I2C，每帧整行/整屏重写，导致：
- 肉眼可见黑闪（尤其抓包页 RX 原始片段行每秒多次变化）；
- 全屏重绘每秒多次抢 I2C 总线，与 AHT20 采样冲突，主循环卡顿（进料/退料迟钝）。

**改造**（采用后台显存 framebuffer 思路，模拟整块 OLED GDDRAM）：
- 新增后台显存 `s_fb[8][128]`（1024 字节）作绘制目标，`s_fb_prev[8][128]` 保存已发送帧；
- 所有 `clear`/`clear_line`/`show_text`/`show_char` **只写 `s_fb`，不再直接发 I2C**；
- 新增 `flush()`：每帧绘制完后调用一次，逐字节比对 `s_fb` 与 `s_fb_prev`，**仅把变化字节经 I2C 发出**（按页扫描、连续变化段一次 bulk 写）；
- 永不整屏 `clear` → 彻底无黑闪；不变区域零 I2C 流量 → 大幅降耗、不卡主循环；
- RAM 代价：多 2KB framebuffer（RAM 用量 67% → 77.6%，仍在 20KB 内安全）。
...
**问题（用户实测反馈）**：
1. 进料时第 0 行 `R` 后面是黑的（数字正常走）——进料包是 `statu_flag=0x03, motion_flag=0x00`（`send_out`），而 `oled_log_rx` 的动作判定漏掉了这个组合，动作缓存永不更新；
...
#### 15.7.7 字模 F8x16 → F8x8（8x8 半高字模，2026-08）

**背景**：开启 `BMCU_AUTO_REBOOT_ENABLE=1`（打印完成软复位）后，全功能固件（OLED=1/AHT20=1/RGB=1）Flash 从 61316（99.8%）再增约 524B，**直接溢出 60KB 上限**（`REBOOT=1` 编译失败）。Flash 已逼近极限，必须再抠空间。

**用户诉求**：不想牺牲功能（不想关 AHT20/RGB），也不想砍字模"分辨率字节"——但实测 60KB 已无法在"全功能 + 8x16 字模"下塞进 REBOOT。最终**用户确认接受把字模从 8x16（每字符 16B，占 2 页）降为 8x8（每字符 8B，取原字模上半截，占 1 页）**，换取全功能 + REBOOT=1 装下。

**改造**（净省约 320B，让 REBOOT=1 得以装下）：
- 字模 `OLED_F8x16[38u][16u]`（608B）→ `OLED_F8x8[36u][8u]`（288B），删去屏幕从不使用的 `W`/`Y` 两个字符，并在 `OLED_FONT_MAP` 相应位处置 0（空位）；
- `show_char()`：原来写 2 页（上 8 行 + 下 8 行），改为只写 1 页（8 字节）；`clear_line()`：`memset(s_fb[row], 0, OLED_W)`（不再 ×2 页）；
- framebuffer 仍是 8 页结构，但 OLED 实际只画 4 行（行 0-3）占页 0-3，**字符高度减半**（128×64 屏理论可 8 行×16 列，目前仍只画 4 行、占上半屏，字变紧凑）；
- 同步清理被 LTO 早已剔除的死代码 `write_data()` / `color_name()` / `motion_label()` 实现与声明。

**验证**：
- `BMCU_AUTO_REBOOT_ENABLE=1 bash build_one.sh standard 1 1 D "" 1 GFU02 GFU95 GFU90 GFU85` 编译通过，Flash 61340（99.9%，余量仅 100B），装下 REBOOT；
- 恢复 8x16 的全功能 + REBOOT=1 不可行：必须关 AHT20（省 ≈1.6KB）或关 RGB（仅省 448B 不够）才装得下；用户选择保留全功能、接受 8x8 字模；
- 代价：OLED 字符视觉高度减半、更紧凑；功能与轮询逻辑零回归。
