...
> **版本号写法说明**：Bambu Lab 打印机固件版本为四段式 `XX.XX.XX.XX`（如 `01.05.00.00`）。本仓库其它文档（如 `BMCU使用指南.md`、`项目说明.md`）常把它简写为三段式（如 `1.07.00` ≈ `01.07.00.00`），二者指同一版本，仅写法不同，不要误以为是不同的版本。
...
对于 A1 和 A1 mini，我们说的是固件 **`01.05.00.00`**（引入授权控制的版本；此后 BMCU 虽会报 HMS 警告但仍可打印，见第 7 节）。
